...
% Maxima pendiente Newton en un mallado de [-10,10)x[-10,10]
%Nombre del archivo: max_pend_newton_multistar
...
fclose(fileID);
function [opt] = max_pend_newton(x0,eps)
 it_pend = 0;
 h = 0.001;
 gx0 = grad(x0,h);
 while norm(gx0) > eps
...
% Proyecto
   % F = sin(x(1)+ x(2))+(x(1)-x(2))^2-1.5*x(1)+2.5*x(2)+1; %McCormick
    %F = ((x(1)^2)/4000)+((x(2)^2)/4000)-cos(x(1))*cos((x(2))/(sqrt(2)))+1;%FUNCIÓN GRIEWANK
